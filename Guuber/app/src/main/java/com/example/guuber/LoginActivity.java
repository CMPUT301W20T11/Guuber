...
public class LoginActivity extends AppCompatActivity implements RegisterFragment.OnFragmentInteractionListener {
	...
	private void updateUI(FirebaseUser user) {
		...
		if (user != null) {
			...
			uRef = db.collection("Users").document(user.getEmail());
			uRef.get().addOnSuccessListener(documentSnapshot -> {
				User loggedUser1 = documentSnapshot.toObject(User.class);
				((UserData) (getApplicationContext())).setUser(loggedUser1);
				Intent homeScreen;
				int signInType = 0;
				if (loggedUser1.getRider() == 1) {
					signInType = 1;
				}
